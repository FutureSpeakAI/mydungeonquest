...
- filter-branch exits 1 doing NOTHING when refs/original holds a prior scrub's backup — delete the ref or -f, then re-run the outcome gates; and its END-OF-RUN CHECKOUT is the worktree-casualty vector (a path stripped from history vanishes from disk) — restore via `git show OLDSHA:path` (2026-07-25, both bit in one sitting).

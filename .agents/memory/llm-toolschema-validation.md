...
## Addendum (July 2026, second strike — the hidden seat)
- The trap has an INSIDE variant: schema **descriptions** that reference a seat the schema never **declares** (three property descriptions said "use this same turn's `world.region_add`" while no `world` property existed). The model faithfully follows the declared shape, cannot emit the referenced seat, the validator refuses, and the error-guided repair retry runs into the same invisible wall — then the silent fallback eats it. Two sealed sittings failing with IDENTICAL refusals is the tell (real model noise varies; an invisible wall repeats exactly).
- **Sweep rule:** every path the validator OR the state fold READS (`story.world?.x`, teach-seats, same-breath exceptions) must be DECLARED in the tool schema. Grep the validator/fold for property reads and diff against the schema's declared properties; a description pointing at an undeclared seat is worse than silence.
- Enums in the added seat import from the one table the fold enforces (never a re-typed mirror), and the declared shape mirrors the fold's tolerances — no stricter (new refusal class), no looser.

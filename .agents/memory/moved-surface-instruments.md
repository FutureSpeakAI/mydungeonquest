---
name: Moved surfaces drag their instruments
description: When a UI surface migrates homes (overlay → new component), every eval needle and court navigation pinned to the old seat must move in the same task.
---
A surface that moves keeps its classes but not its page: every read()-pinned eval needle and every court that walks to it must be retargeted in the same task the surface moves, then proven by the keyless tail AND a loop sitting.

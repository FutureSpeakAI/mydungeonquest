...
## Live keys auto-engage (learned July 15, 2026 — the review-shoot incident)
Adapters PICK A LIVE PROVIDER AUTOMATICALLY when its key is present in the env; PAINT_PROVIDER only forces a choice. In this workspace the paint keys ARE set, so ANY process that calls adapters() — scripts, helpers, review/testing subagents executing "just to validate" — paints with real money unless explicitly pinned to mock. A review subagent executed the brand shoot mid-review and spent ~14 live frames before its window closed.
**How to apply:** every automated or delegated context that might execute adapter-touching code must pin PAINT_PROVIDER=mock (and siblings) in its env or instructions; only a direct owner request runs live. The brand shoot itself now walks the Watchtower toll gate (per-frame ceiling check + tally mark) and resumes past frames already on disk, so a rerun never repaints paid art.

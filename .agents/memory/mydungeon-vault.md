...
- **No marker rows on the media shelf.** PK = assetHash (content-addressed) and sync chain-verifies rows against bytes fail-closed: a byte-less or pseudo-hash "refused/terminal" row is structurally a forgery and can overwrite an anchor. Terminal outcomes live in the sealed attestation record; if durable terminal observability is ever needed, it must be a separate cacheKey-keyed resolution record, never a media row.

...
**Vision cache ignores question text.** The verdict cache key is sha256(bytes + sit id + protocol tag) — the question prose is NOT in it. Editing a judge question does NOT invalidate old verdicts: crossed verdicts replay false forever unless manually evicted (scan cache JSONs by sit id, unlink). Protocol-tag bumps DO re-key everything — but if the tag rides the paint-law digest, bumping it razes the whole plate store (paint lottery re-rolled). **How to apply:** after any question edit, evict the specific crossed entries by id; never bump the protocol tag for a judge-law-only change.

**Paint-law vs judge-law raze boundary.** The store-freshness hash covers only the PAINT law (prompts, foundry briefs, warden). Judge questions are judge law — editing them leaves the store standing (good: seasoned verdicts keep replaying; only evicted sits run fresh).

## Task 61 close (July 2026) — the tank, the eyes, the seats
- **Probe the provider tank before flagging a counted seat.** Live-tier courts rightly REFUSE to judge on fallback when a provider account empties (Anthropic "credit balance too low"), and the red spends the seat all the same — a window died on arithmetic after exactly this. A one-token health call (status-only) before each counted flag is free; an outage mid-seat costs the seat. Rehearsals passing minutes earlier prove nothing about the tank NOW.
- **The eyes checklist is EVERY court-examined plate class, per criterion.** Anchor (mark form + face + headroom), hero sheet (mark in closeup), each scene (per named subject: whole/uncut at frame edges; mark either clearly canon-shaped or honestly absent behind cloth — a formless blotch is the losing band between visible-pass and attested-lack-pass), villain intro/later/sheet (masked, mutually consistent), supporting-soul sheets, place sheets/regions, keyart. Roll 6 fell on an uneyed villain sheet; roll 7 on scene subtleties (blotch mark, edge-cut second soul) after eyes had only cleared the "hero classes".

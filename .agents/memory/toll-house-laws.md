...
- **THE ONE LEDGER SEAM (Law XIII, 2026-07-25)**: the users rows ARE the entitlement ledger and carry `plan_source` (default 'stripe' via additive DDL; appstore/play rows arrive with Directive XXI and must write rows, touch nothing else). EVERY standing decision — innkeeper grant, checkout comp/already-seated checks, reconcile gift check — folds through the ONE pure predicate `entitledStanding(rows)` in toll.js: highest lawful seat wins, malformed/no rows → 'free' (single 'guest' row still seats guest via the null-start fold), and it NEVER reads plan_source or the customer id (the gate courts its toString). Storefront fields are lawful only at storefront doors (mint-skip in checkout, portal). The Stripe writer stamps `plan_source = 'stripe'` in its UPDATE; that verse, the grown standing read (`SELECT plan, plan_source, stripe_customer_id …`), and tollhouse's point-of-sale needle all have whole-verse twins (oneLedger gate + tollhouse) — grow any of them only by re-seating the twins deliberately. `__standingConsultsForEval` telemetry proves the only-road in court; a new standing door that skips the predicate reds the counter court.

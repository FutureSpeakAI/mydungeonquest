...
- Twelve table gates keep no engine twin, and honestly: book and boundary
  ...
  manifest; the engine ships no bundle), and secondChair (the Second
  Chair seats the room's chairs on the table's own provider envs behind
  the server door — the room is server law; the engine keeps no
  provider seats), and cellar (the cellar sweep plans and executes
  over the table's own Dexie media shelf — client-store law proven
  under fake-indexeddb; the engine keeps no shelf), and pyreParcel (the
  owner's two rights — the parcel and the account pyre — bale and burn
  the house's own Postgres shelves and blob store behind the named
  server door; the house's stores are server law, and the engine keeps
  no stores), and oneLedger (the one ledger seam folds the toll's
  standing rows — source-stamped by additive DDL in the house's own
  Postgres — through the single entitledStanding predicate at every
  pour, taste, and seat door; the engine keeps no toll and no
  ledger). The room law those
...
- The engine suite stands alone: the proving campaign fixture lives at
  engine `evals/fixtures/`; no gate reaches across the monorepo.
  Directives VII–XIV are shelved at `packages/engine/docs/directives/`
  (see that shelf's README for the two lineages at IV–VI).
- The back-compat gate seals the port's oldest promise: a pre-0.7 save
  walks every reader deep-frozen — loading writes nothing, absences read
  as absences, reducers upgrade only the copy, and the mock DM plays the
  old tale whole.
- The desk gates of §2 moved both counts by one apiece; fifty-eight and
  125 stand at the 1.0.0 tag.

## 0.6.5 — The First Word (2026-07-17, Task 54C)
- CLOSED GREEN under THE EXTENSION LAW (first application): the
  original ceiling of four ran .1 ✓ .2 ✓ .3 ✗ .4 ✓ — the sole red was
  the court's own wire assertion misreading a forge preview straggler
  as a genesis paint (the cure was never implicated; that run's first
  byte was ~250ms), amended stricter: forge lane drained before the
  tap, the first-word law absolute, violators named by lane. The house
  granted .5–.7 by ruling; all three ran GREEN, 61/61 courts, zero
  skips — three consecutive fully green runs inside the window, under
  code freeze, one strike never invoked.
...
- The architect round (FAIL → three cures: the queueMedia meter
  settles once per call as a delta after all lanes settle; the
  prologue delta merges unconditionally; the sequencer contains
  synchronously-falling pours) is BANKED — the cures ran under all
  three extension iterations.
...
| K8 | 30-turn long march | k8-longmarch.spec.ts: 24/30 turns landed (6 timed out waiting for composer); floors written — narrationWords ≥ 2565, platesRendered ≥ 22, safeFallbackTurns = 0, narrationFloorBreaches = 0; 3 bugs fixed (beatMeasure skip, dialogue_cue null, padWords ceiling escape) | k8-budget.json floors committed; ritual-dismiss guard added |

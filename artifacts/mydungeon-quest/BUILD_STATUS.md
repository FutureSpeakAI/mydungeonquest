...
- One truth, two shelves (this section is pinned verbatim in both repos'
  ...
  table's keyless check stands at 127 green verdicts (`npm run check` in
...
- Six table gates keep no engine twin, and honestly: book and boundary
  ...
  with the provider seats), and pensClock (the pen's clock races those
  same provider fetches at the transport — the engine keeps no wire to
  bound). The room law those doors import is proven by the engine's own
  gates (director, curtain, editorEcho, editorCliche, firstWord).

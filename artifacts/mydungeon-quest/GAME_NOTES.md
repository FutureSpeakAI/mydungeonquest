...
- THE CHRONICLE LAW (minted 2026-07-20, the reconciliation before Task 63): no commit may move the pin, the version, or law bytes without its CHANGELOG entry and LOOP_LOG chapter landing in the same task, whoever commissioned the work; a predecessor found silent is reconciled with provenance before any successor builds; every future precondition check runs pin first, narrative second — pin presumed true, narrative repaired on divergence.
- THE WHOLE RITUAL LAW (minted 2026-07-20, beside the Chronicle Law): the closing sentence may not be spoken and the closing commit may not land until every step of the ritual has its artifact in the ledger; the verdict machinery carries this check wherever it is mechanical (the keyless `wholeRitual` gate balances closed CHANGELOG versions against `RITUAL COMPLETE` lines in LOOP_LOG), so a missing sitting refuses the sentence by itself. A close later found missing a step reconciles as on 2026-07-20: held-late, with provenance, under the severity-conditional disposition — clean or refuted stands; cured instrument-side or in provably-untraveled paths stands with targeted proofs; severe in traveled game or law bytes voids those greens and the window reopens bounded.
...
## THE PODCAST FORGE (the Experience Cut, Phase 5)

One produced episode from the sealed record, downloadable as MP3 from Settings.

- **The script is compiled, never written.** `buildPodcastScript` (`src/lib/podcast.js`): cold open — the highest-stakes sealed quote, in its own character's voice — then the title liturgy, then one section per walked beat: the Chronicler's passage read whole by the narrator, the cast re-speaking their finest sealed lines verbatim. A beat with no page falls back to selected sealed narration (cinematic-adjacent turns, high-stakes rolls, bond swings — the episode is a retelling, not a replay). Sign-off: "Sealed, and true." The liturgy is the ONLY unsealed text, and it is a template. `validatePodcastScript` is the verbatim court: every segment must match its cited source exactly; redaction is journal law first, snapshot flag second — same two courts as the Storybook.
- **The mix cannot overlap.** `buildMixPlan` emits voice/gap/sting items — stings (the tale's own sealed musical phrases, real provenance only) sound only BETWEEN sections, framed in 900ms of silence. `assertLawfulPlan` refuses any two audible items that touch, and the server re-judges every plan at its own door. `server/sequencer.js` renders ONE ffmpeg concat chain — generated-silence gaps, loudnorm −16 LUFS voices, −19 stings — and contains no way to mix two sounds at once. The old music bed is retired: a `bed` request is refused by name (400).
- **Provenance at the door.** Mock audio anywhere in the payload → 422. Keyless tables get the honest decline: "The podcast needs voices. Add a key, or keep the book."
- **Garnish rides along, never blocks.** Chapter markers (ffmetadata on the true timeline, ffprobe durations; a failed probe skips markers, never the episode) and the latest key art as the attached-pic cover, through a strict data-URI door.
- **Proving:** `evals/podcastForge.test.mjs` — verbatim court, tamper refusal, journal-law strikes, lawful plans, and the no-amix graph.

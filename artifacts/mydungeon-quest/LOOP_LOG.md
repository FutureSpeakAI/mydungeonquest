...
## TASK 61 — THE COMMONS (Directive XV, game 1.1.0)

### The commons take the bench and the court (2026-07-19, seating — every test and rig edit, ledgered)
- THREE GATES join the keyless eval (§V pin 125 → 128, ledgered in g13-check.spec.ts the same commit):
  - `evals/mirror.test.mjs` NEW — the vault as a true mirror and the publish desk on the bench: keyless dormancy (and the seam DEAD wherever the real door stands); the staging seam judged in keyless PARTS (inscription lane through an injected visitors' book; the doorless doorkeeper passing guests, and stumbling LOUDLY into guesthood when the seam names a patron no ledger can seat — every Postgres coordinate scrubbed so the pool is provably unreachable); crash-retry convergence byte-for-byte; append-only 409; restore fidelity through the engine's own desk; the publish refusals each by name (wax 422, tampered chain 422, unshelved plates 422 with the missing list); one living page 409 naming the standing page; public doors serving GUESTS (meta no-store, record bytes VERBATIM including a planted spacing oddity, plates on the modest hour cache, commons honoring the unlisted default); owner rooms 401 fail-closed; revocation a 410 tombstone at every door, idempotent, unlistable. The whole-seam walk (doorkeeper → stagePatron → real ledger) is deliberately left to G29's live house — the bench judges parts it can hold keyless, the court walks the whole.
  - `evals/publishRules.test.mjs` NEW — the pure laws: only the wax licenses publishing; the published record is the EXPLICIT four-field envelope (header, campaign, journal, media) minus blobs and volatile URLs — the seen ledger can never ride by construction; strikes fell by HASH so an interleaved hashless tick row cannot spring the ordinal trap (this bench plants exactly that trap); a felled row keeps nothing but seat and stamp; the page model carries no struck byte; a tombstone is a refusal and only a refusal.
  - `evals/keyHome.test.mjs` NEW — the key stays home: a SENTINEL private scalar seeded into the key row, then every egress deep-scanned — export envelope, captured vault push bodies, published record. Any `d`, any privateKey field, any live CryptoKey, any sentinel byte, or any UNSCANNABLE value (function, symbol, bigint) fails the gate; the PUBLIC half is asserted PRESENT on all three (the wax needs it). A live-signer sanity leg proves a freshly minted campaign exports clean too.
- `package.json` — the eval chain grows the three gates (rig edit, same commit as the pin).
- `tests/e2e/g13-check.spec.ts` — PINNED_PASS_COUNT 125 → 128 with the §V ledger line (G13 ledger law: the pin moves in the same commit that seats the gates).
- G29 — THE COMMONS court, NEW (`tests/e2e/g29-commons.spec.ts` + `tests/e2e/lib/commonsServer.ts` + `tests/e2e/fixtures/commons-tale.json`):
  - The commons house raiser: doorless (Clerk scrubbed so the staging seam may stand), every AI provider scrubbed (the court seeds its plates, it never pours), REAL ledger and REAL plate shelf kept — and it REFUSES to stand without them: a commons court that silently skipped would be a mock door. Ports 5193/5192, staged patron `g29-commons-court`; the project rides behind g26 so its house never stands beside a keyed one (playwright.config.ts, same commit).
  - The fixture: "The Lantern of Bell Hollow" — eight PG turns, tick-free by design (no combat, no clocks: the redaction law's ordinal traps are the BENCH's business; the court walks the honest road), one struck turn whose sentence appears nowhere else, one kept sentinel sentence, one deterministic 393-byte plate on an UNSTRUCK turn (its SHA pinned in the court for the shelf poll), three named souls for the dramatis personae.
  - The walk: seed sealed → the mirror fills (journal polled to the sealing record; the plate polled onto the shelf) → publish from the ceremony → a GUEST context reads /t/<id>: badge EARNED in the browser (data-verdict=true), the book breathes the kept sentence, the struck sentence NOWHERE on page or book, seven episodes, four souls, the plate hanging with real pixels — while the RAW record still carries the struck bytes (the chain is the law) and knows no 'reveals' (the seen ledger never rides) → a SECOND device restores from the shelf and the wax re-verifies → revoke → tombstone: 410 at record and plate doors, no tale content on the stone. The court sweeps its staged patron's living pages at start — it owns its own 409s across loop iterations.
- TOOTH 17 — THE FORGERY (calibration.spec.ts APPEND, own house 5191/5190, patron `tooth17-forgery`): the badge itself on trial. A lawful hash-only chronicle minted at the bench publishes clean and earns data-verdict=true; the same page read through a route that flips ONE letter of prose in the served record must earn data-verdict=false with the refusal spoken. Perfect separation or red — a badge that stays green over forged bytes is worse than no badge. Sweeps and revokes its fixed-id page so no living page outlives the sitting.
- `tests/e2e/webserver.sh` — strips VAULT_STAGE_PATRON alongside the door keys (rig edit): the rig's shared app server stays a doorless GUEST house; only the commons courts' own houses name the seam.
- Client seats this task (for the record; the courts above judge them): `src/lib/publish.js` (pure core + shell), `src/components/PublicTale.jsx` + `/t/<id>` route in `src/main.jsx` (strict-door id regex at the boundary), the ceremony's commons panel (hidden on dormant tables — the keyless loop courts see zero new UI), `src/lib/vault.js` restore desk court (chain always; signatures only on a 'signed' claim — a restored-then-continued spine lawfully wears hash-only over old signed ink, so the whole-envelope downgrade door would refuse the player's own tale), `src/lib/proving.js` additive `plate` field (bytes hashed OUTSIDE any Dexie transaction, per the crypto-in-transaction law).

### 61.1 red — the court died at collection (2026-07-19)
- run-iter61.1 fell at the DOOR, not the bench: g29-commons.spec.ts read its fixture via `__dirname`, which does not exist in this rig's ES-module scope — the collection error felled EVERY project before one court sat (verdict: zero tests, all projects missing). Mechanical, not material.
- TEST EDIT, ledgered: the fixture path now rides GAME_ROOT from lib/vision — the rig's one proven way to name its own floor. Swept every new file for the same trap (none). Rerun as 61.2.

### 61.2 red — the tooth caught a REAL door, and the court knocked without the key (2026-07-19)
- run-iter61.2: 76 green, 2 red (tooth 17's forged sitting; G29 at the seed), 28 downstream unrun. Two crossings, two different families:
- TOOTH 17's forged sitting stayed GREEN-badged — and inspection showed the tooth had found a real laundering seam, not a bench slip: the prose lives TWICE in a published record (the journal's hashed payload and the campaign's display copy), my single `.replace` forged only the DISPLAY copy, and the chain court alone verifies the JOURNAL — so forged display prose could stand beside a true badge. THE CURE IS APP LAW, not bench law: the display-coherence court (`displayCoherence` in lib/publish.js, pure, fail-closed) — every DISPLAYED row must cite its journal record by hash and match it canonically (teller's words AND player's deed); citeless, unknown-cite, diverged, or unreadable ⇒ the badge falls whole. Wired into the reader's verdict AFTER the chain court (fetchPublicTale); Directive XV §III.4 amended in this commit (one line: the badge answers for the words the page shows). Meta stays presentation (title/shelf face move without reseal — the vault's meta-push law); the STORY's words are the chain's or they are nothing.
- TEST EDITS, ledgered: tooth 17 now tries the forgery TWICE — `replaceAll` (both copies; the chain law itself must break the badge) and single-`replace` (display only; the coherence court must break it). publishRules gains court 6 (lawful record coherent; one forged display byte, a forged spoken deed, a citeless page, a stranger's cite, and a null record each fell it — assertions only, no new PASS line, pin stands at 128). The bench's specimen journal was minted with EMPTY payloads — under the new law an envelope whose copies disagree is rightly incoherent, so the specimen was made honest (journal payloads carry the words the logs display, as every real export does).
- G29 fell at `window.__mdqSeed is not a function`: the seed door only exists on the proving road and the court knocked at `/` plain. Cure: the court now walks boot()'s own way — arrival curtain skipped by init script, `/?proving=1`, title page awaited — on both the author's and the restorer's devices. Rerun as 61.3.

### 61.3 red — one crossing, and it was the page smuggling the record (2026-07-19)
- run-iter61.3: 105 green, ZERO skips, one red — G29 alone, at its own letter: the struck sentinel stood in the guest page's DOM. Not rendered — smuggled: the public book was bound with the OWNER's keepsake device, whose `chronicle-proof` node embeds the whole byte-preserved record (struck bytes and all) into the iframe srcdoc. The rendered prose was clean (the book's own innerText assertion passed); the machine proof was the leak. Everything upstream held: both tooth-17 sittings (chain forge AND display-only launder both felled the badge), the mirror, the publish walk, the badge earned true in the guest's browser.
- THE CURE IS APP LAW: `buildStorybook` gains a `proof` option (default TRUE — the owner's keepsake is untouched, its save-proof device and embedded chronicle ride exactly as before); the public reader binds with `proof: false`. A guest's rendered page now holds not one struck byte; the byte-preserved record lives behind the commons' record door, one honest fetch away — §III.6's two halves (the RECORD keeps struck bytes, the PAGE fells them) no longer share a DOM. No test or rig file changed this iteration; the spec's letter stood and the app rose to it.
- Keyless check re-run after the cure: exit 0, exactly 128 PASS. Rerun as 61.4.

### 61.4 red — one guessed name, and two single crossings under a loaded run (2026-07-19)
- run-iter61.4: 75 green, 3 red, 28 downstream unrun — and the run itself ran HOT (30.7m against 61.3's 13.2m; harvest alone 9.5m).
- G29 fell at its own needle: the cast tab's face reads "Dramatis personae", and the court had knocked with a guessed accessible name (/cast/i). Everything the 61.3 cure touched HELD — the guest page carried not one struck byte, the badge earned true, episodes listed. TEST EDIT, ledgered: the cast needle now holds the surface's stable instrument (`tab-cast` testid), not a guess — the moved-surface law applied to a surface that never moved but was never read.
- Tooth 11 (calibration probe separation) and G11b (clean plates) each crossed ONCE — both greened in 61.2 AND 61.3, both are vision-judged courts, and the run was anomalously loaded. The one-crossing law holds: not a conviction; no court is touched; fix the rest and rerun. A second crossing in 61.5 becomes material.
- Rerun as 61.5. Iterations spent: 4 of 8.

### 61.5 red — the shelf told the truth, and two courts crossed twice (2026-07-19)
- run-iter61.5: 75 green, 3 red, 28 downstream unrun (11.7m — the 61.4 heat did not return).
- G29 progressed past the cast tab and fell HONESTLY at the vault shelf: three spines of "The Lantern of Bell Hollow" stood side by side — one per iteration's walk, each a distinct campaign under the same staged patron. The court had knocked by title. APP+TEST EDIT, ledgered: the spine now carries `data-campaign` (an instrument, additive) and the court holds ITS OWN campaignId — a patron's shelf may lawfully hold many same-titled tales; a court may only ever claim its own.
- Tooth 11 and G11b crossed a SECOND consecutive time — material under the standing law. The sealed evidence was inspected directly (both plates read with the agent's own eyes, prose beside plate):
  - tooth 11, `good-moment-live/plate-cea96a4bad.jpg` (no_contradiction refused twice, byte-identical): the plate seats EVERY promise of its prose — dusk, the bending cobbled road, Mara Vey with chalk-bright fingers, the unrung bell, the stilt-legged houses, the hero foremost. A judge over-read on a true pairing.
  - G11b, `live/Mara_Vey-62edcc6b92.jpg` (contains_text 0.82 twice, byte-identical): a faint signature-like smudge sits at the plate's bottom-right that inspection could NOT conclusively clear. An unclearable plate is never whitelisted.
  - Both convicted byte-runs were minted in ONE sitting — 61.4's hot harvest (the pool's mtimes prove no re-mint between 61.4 and 61.5; the same bytes were judged twice). `plate-f4506a4e6b` crossed once only (61.4) and stays seated — one crossing is not a conviction.
- THE CURE IS CUSTODIAL, NOT A WHITELIST: the live pool is razed (the store's own self-expiry law — harvest A re-mints the whole live tale when its seat files are absent; preflight gates keys, never the store). JUDGE_BOUNDARY stays EMPTY: quarantine binds to bytes, the bytes die with the raze, and no entry may outlive its evidence. If a successor pairing crosses twice fresh, the two-sitting law applies then — with this inspection ledgered as precedent.
- Keyless check after the app edit + raze: exit 0, exactly 128 PASS. Rerun as 61.6. Iterations spent: 5 of 8 — the remaining budget is exactly the three consecutive greens owed. No spare.

### 61.6 red — the anchor wore its key on the wrong canvas (2026-07-19)
- run-iter61.6: ONE red, 77 green, zero flakes — and the walls held everywhere the last cures touched: G29 walked its WHOLE court green for the first time (mirror → publish → guest badge true → episodes → cast by instrument → second-device restore by its own spine → keepsake → revoke → tombstone → 410s), and tooth 11 + G11b greened on the fresh mint, vindicating the raze-not-whitelist ruling.
- The one red was tooth 12: the freshly-minted hero anchor refused the magnified look (`mark_visible:false`, 0.82). INSPECTED with the agent's own eyes: the painter rendered the key as a WROUGHT-IRON EMBLEM on the pauldron — armor decoration, not "a burn in the shape of a key" on flesh. The magnifier refused HONESTLY. A defective mint, not a judge miss.
- The tempting cure was REJECTED: strengthening HERO.mark to name its canvas ("on her bare shoulder") would sharpen the painter's brief — but g22d binds the SEALED fixture record's prompts to `m.hero.mark` verbatim, so the constant cannot move without razing the fixture pool too, and a double re-mint multiplies reroll dice across every fixture-judged court with zero spare budget. The phrasing stands; the mint rerolls.
- RIG EDIT, ledgered: run-loop.sh gains an out-of-ladder `premint` flag value — the workflow mints the harvest pool ALONE (harvest --no-deps), writes premint.log/premint.exit, consumes NO iteration number, and parks. Two detached-shell attempts were reaped mid-suite (the runner's own founding law re-proven); the mint must ride the supervised workflow like everything else. The next numbered run reuses the pool under the same paint-law reuse every run honors.
- THE PRE-MINT CYCLE, all sittings ledgered: mint 2 placed the key as a brand on the leather MANTLE — inspected borderline, then MEASURED by running tooth 12 itself out-of-band against the standing pool (the exact court, question, and bytes; no iteration spent): RED. Razed. Mint 3 seared the key-shaped burn into the SKIN of her neck — inspected TRUE to the letter, probed: tooth 12 GREEN (fresh judge sitting, 15.4s). The pool stands measured-approved.
- THE EXTENSION, declared and FROZEN: ceiling 8 → 9, the standing one-time extension, spent here. Six iterations are gone; three consecutive greens are owed; 61.7 and 61.8 sit within the old ceiling and 61.9 is the extension's seat. There is no second extension — if any of the three reds, the task reports honestly instead of closing.
- Rerun as 61.7 on the approved pool.

### 61.7 red — the beheaded control stopped lying (2026-07-19)
- run-iter61.7: ONE red, 77 green, zero flakes. G29 walked its whole court green AGAIN (15.8s — second consecutive full passage), tooth 12 greened on the premint-approved anchor (the measured pool held), and every prior cure stood. The red was tooth 11's perfect-separation law: known-bad pair `bad-beheaded-hero` PASSED all binaries.
- DIAGNOSED WITH OWN EYES, crop reproduced exactly (top 40% of 896×1200 cut, keeps y=480..1200): on the fresh anchor the head sits LOWER than the control's geometry assumes — the crop retains mouth, chin, and jaw, and the key-burn on her NECK rides dead center, plainly visible. The caption's lie ("whole face in view, mark plainly visible") became half-TRUE of the cropped bytes: the mark clause is emphatically honored and a partial face IS in view. The pinned binaries never caught a contradiction because the fair eyes found none worth refusing. THE JUDGE WAS HONEST; THE CONTROL BROKE. Same family as the retired crossing control (56B.4): a control that flips on a fair judge is a dice roll, not a lie — "known-bad controls must be deterministic lies, never crossings."
- THE CURE, designed and ledgered though NOT landed (no iteration remains to serve it): make the control self-verifying — derive the behead line from the DETECTED face box (the magnifier's stage-one instrument already exists) so the crop provably removes the whole face on ANY composition, and aim the prose at what the crop provably cuts (eyes, brow, gaze — DROPPING the mark clause, whose truth floats with the painter's placement whim). A control that proves its own lie before seating is a strengthening in the exact direction the standing law demands.
- THE ARITHMETIC, faced squarely: seven of nine seats are spent (61.1–61.7; the one-time extension was declared and frozen at 61.6). Two seats remain; three consecutive full-loop greens are owed. The ritual CANNOT complete within the frozen ceiling. Per the 61.6 declaration — "if any of the three reds, the task reports honestly instead of closing" — the loop halts here pending the owner's ruling. No further seat is spent unbidden.
- What stands proven regardless: the Commons body of work greened end-to-end twice consecutively (G29 in 61.6 and 61.7 — mirror, publish, guest verify, episodes, cast, second-device restore, keepsake, revoke, tombstone, 410s); the premint door mints material out-of-ladder for inspection and probing before a numbered seat is risked; three reds across 61.5–61.7 were three DIFFERENT fresh-material boundary events with zero repeat convictions.

### Task 61 halted by the owner's ruling — the Beta Cut takes the road (2026-07-19)
- The owner ruled on the halted ritual not by answering the ceiling question but by handing down TASK 60B: THE BETA CUT (1.0.1), whose own precondition governs the unclosed-61 world: start from c2b1b1f (the Task 60 sentence, v1.0.0, pin 125). The Beta Cut is authored PRE-Commons by its own words ("no cloud exists until the Commons lands... cloud and billing arriving as their own tasks after").
- Task 61 closes UNCLOSED, honestly: seven of nine seats spent, three consecutive greens unreachable, nothing weakened to pretend otherwise. What its runs proved stands in this history: G29 walked the whole Commons court green twice consecutively; the premint door mints material out-of-ladder for inspection; the display-coherence, proof-gating, and instrument-needle cures all held.
- This commit is tagged task-61-halted — the Commons returns as its own task after the beta, rebased on 1.0.1, its ritual begun afresh. The owner published the app at 80d7484 with the Commons aboard; that production snapshot is theirs and outlives this tree's return to c2b1b1f.
- Next commit restores artifacts/mydungeon-quest to c2b1b1f exactly. The 61-era LOOP_LOG chapters live here and in the tag, not in the restored file — each era keeps its own ledger.

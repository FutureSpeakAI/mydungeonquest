// THE BESTIARY GATE (Directive X, Laws I–II) — species canon seals once,
// like fixture canon: the door rules shape and the seal court refuses a
// rewrite by name; every instance derives from THE THREAT TABLE through
// the ONE expansion; the block carries bestiary_state; the easel rides
// up to three sealed visuals of the LIVING battle, most recently sealed
// first, byte-stable. Keyless, deterministic.
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { expandSpawn, makeEntropy, THREAT_TABLE, validateDmTurn } from '../src/lib/protocol.js';
...
// ---------------------------------------------------------------
// VII. THE SCHEMA MIRROR — the model sees the law it is judged by.
// ---------------------------------------------------------------
const dmSource = readFileSync(new URL('../server/dm.js', import.meta.url), 'utf8');
for (const needle of ["creature_add: { anyOf:", "required: ['species','visual','nature','threat']", "spawn: { anyOf:", "initiative: { anyOf:", "required: ['device','entropy']", "required: ['actor','action']"]) {
  assert.ok(dmSource.includes(needle), `the tool schema mirrors the strict law: ${needle}`);
}

console.log('PASS — bestiary law: the seal, the threat table, the one expansion, the block, and the painted rider hold.');

// THE COIN-PURSE GATE (Directive VI) — narrative coin as sealed movements.
// Every movement carries holder, signed whole delta, and honest reason;
// the overdraft is refused at the door (folding the turn's own movements
// in order) and clamped at the fold; the replay reaches the same figure
// from the journal alone; and the briefing speaks the hero's wealth in
// one deterministic line that trims only after the allegiances are gone.
...
import { purseOf } from '../src/lib/trove.js';
...
console.log('PASS — the coin-purse gate: coin moves only by sealed reasons, the overdraft is refused at the door and clamped at the fold, every purse replays to the same figure, and the briefing speaks the standing wealth in one honest line.');

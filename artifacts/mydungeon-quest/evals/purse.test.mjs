// ------------------------------------------------------------
// THE PURSE GATE (game) — the Ledger Law's first seat
...
const { buildLedger, assertConservation } = await import('fatescript/ledger');
const { tradeRowsFrom, heroPurse, openingStake } = await import('../src/lib/ledger.js');
...
// 5. THE WIRING — the engine's one ledger under the lib, the folio's
...
console.log('PASS \u2014 the purse gate (game): trade rows derive from the sealed record with stable ids and causes in the turn\u2019s own words, struck and offscreen rows move nothing, the folio\u2019s coin and pack are byte-identical projections of the engine\u2019s fold, a causeless windfall and an overdraft are receipts rather than coin, drift from the sheet\u2019s old memory is named out loud, and the conservation court balances the room.');

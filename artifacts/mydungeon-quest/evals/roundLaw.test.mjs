// THE ROUND-LAW GATE (Directive X, Laws III–IV) — combat opens by sealing
// initiative as an operation: the device side named whole, one accounted
// d20 pool draw per enemy species group, at most three; one action per
// living combatant per turn, a second refused BY NAME; the sealed order
// never reshuffles — ties break toward the player, then alphabetically;
// the downed and the fled keep their seats. Keyless, deterministic.
...
console.log('PASS — round law: the sealed opening, the accounted draws, the device side, the one-action court, and the tie law hold.');

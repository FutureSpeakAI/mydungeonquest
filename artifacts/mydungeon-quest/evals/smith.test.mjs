// THE SMITH GATE (Directive XIII §5) — the candidate court. The strict
// validator refuses locked keys, stranger keys, missing keys, incoherent
// callings, and unswept text; the tool schema MIRRORS the validator
// (enums included) so the live smith cannot deal a valid-but-rejected
// set; the mock floor is lawful by construction across every scope,
// field, and lock; and the temperature is pinned.
...
// —— (58B review) The governing calling may sit in the LOCK. ——
// The architect's case: a whole-hero deal under a locked className
// carries no className of its own — the court must read the lock, or a
// Wizard walks with a greatsword's die and the court says nothing.
...
for (const [scope, field, lock] of [['world', null, {}], ['world', null, { tone: 'held' }], ['hero', null, {}], ['hero', null, { className: 'Wizard' }], ['field', 'className', { bearing: 'held' }], ['field', 'tone', {}]]) {
  ...
  // (58B.3) The fence rides twice from ONE seat: every offered free-text
  // string carries a finite maxLength AND a description naming that same
  // bound — bare maxLength proved advisory to the live model, and a
  // fence the model never reads is a fence only the redraw law enforces.

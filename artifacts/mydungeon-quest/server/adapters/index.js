...
export function providerChains() {
  ...
  const byName = { gemini, openai, elevenlabs: eleven, mock: mockAdapter };
  ...
  const chains = {};
  for (const kind of ['paint', 'speak', 'music', 'sfx']) chains[kind] = chain(kind);
...
export function adapters() {
  ...
  for (const kind of ['paint', 'speak', 'music', 'sfx']) out[kind] = chains[kind][0];
  return out;
}

...
export const mockAdapter = {
  ...
  async paint({ prompt, kind = 'scene' }) { return { bytes: svgBytes(`MOCK ${kind.toUpperCase()}`, prompt, kind === 'portrait' ? 768 : 1280, kind === 'portrait' ? 1024 : 720), mime: 'image/svg+xml', provider: 'mock', model: 'procedural-svg', seed: hash(prompt).slice(0, 12), usage: { cost: 0 } }; },
  async video({ prompt }) { return { bytes: svgBytes('MOCK CINEMATIC KEYFRAME', prompt), mime: 'image/svg+xml', provider: 'mock', model: 'animatic-keyframe', seed: hash(prompt).slice(0, 12), usage: { cost: 0 }, posterOnly: true }; },

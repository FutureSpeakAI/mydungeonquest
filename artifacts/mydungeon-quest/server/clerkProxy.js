/**
 * Clerk Frontend API Proxy Middleware — plain-JS port of the workspace
 * template (this server is untyped ESM; behavior is kept verbatim).
 *
 * Proxies Clerk Frontend API requests through our own domain, enabling Clerk
 ...
 *
 * x-forwarded-host can take three shapes: undefined, a single string, or a
 * comma-delimited string / string[] when an upstream appended rather than
 * replaced the header. In the multi-value case the leftmost value is the
 * original client-facing host. Exported so index.js (clerkMiddleware
 * callback) and this proxy agree on which hostname is canonical —
 * otherwise multi-domain/custom-domain flows break.
 */
...
export function clerkProxyMiddleware() {
  ...
  return createProxyMiddleware({
    ...
    on: {
      ...
      // so Content-Encoding is preserved. Length-known responses (e.g. /npm/*

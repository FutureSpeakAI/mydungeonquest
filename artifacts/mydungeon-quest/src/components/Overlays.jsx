...
// Loads a campaign's painted portraits (by soul name) and region plates
// (by region name) as live object URLs — the gallery Codex wears the real
// art over its monograms wherever the Foundry has painted.
function useGallery(campaign) {
  const [gallery, setGallery] = useState({ portraits: {}, plates: {} });
  useEffect(() => {
    ...
    (async () => {
      ...
      const portraits = {}, plates = {};
      for (const soul of campaign.codex.cast) {
        const row = latestFor(soul.name, 'bust') || latestFor(soul.name);
        if (row) { const u = URL.createObjectURL(row.blob); urls.push(u); portraits[soul.name] = u; }
      }
...
export function Codex({ campaign, onClose, onReplay }) {
  ...
  return <Frame title="The Codex" icon={<ScrollText/>} onClose={onClose} wide>
    ...
    <h3>Souls</h3><div className="codex-grid gallery">{c.cast.map((soul)=><article key={soul.id}>{gallery.portraits[soul.name] ? <div className="codex-portrait" style={{backgroundImage:`url("${gallery.portraits[soul.name]}")`}}/> : <div className="procedural-portrait">{soul.name.split(' ').map((x)=>x[0]).join('')}</div>}<span className="role-tag">{soul.role}</span><h4>{soul.name}</h4><p>{soul.visual}</p><small>Bond {soul.bond}/4 · {soul.status}</small></article>)}</div>
    <h3>Regions</h3><div className="region-list gallery">{c.regions.map((region)=><article key={region.id} className={gallery.plates[region.name] ? 'has-plate' : ''}>{gallery.plates[region.name] && <div className="codex-plate" style={{backgroundImage:`url("${gallery.plates[region.name]}")`}}/>}<b>{region.name}</b><span>{region.state}</span><p>{region.visual}</p></article>)}</div>

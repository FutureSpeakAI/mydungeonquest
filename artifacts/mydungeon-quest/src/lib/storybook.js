...
export function buildStorybook({ campaign, journal, media = [], pageSize = 'Letter' }) {
  // Every plate passes the binder's door once, here — downstream code only
  // ever sees lawful data:image URIs (and esc() belts them in attributes).
  const paints = media.filter((m) => m.kind === 'paint' && safeDataUrl(m.dataUrl));
  const latestByLabel = (name) => paints.filter((m) => String(m.label || '').includes(name)).sort((a, b) => b.createdAt - a.createdAt)[0]?.dataUrl || null;
  const bustFor = (name) => {
    const mine = paints.filter((m) => String(m.label || '').includes(name)).sort((a, b) => b.createdAt - a.createdAt);
    return (mine.find((m) => m.variant === 'bust') || mine[0])?.dataUrl || null;
  };
  const keyArt = paints.filter((m) => m.label === 'keyart').sort((a, b) => b.createdAt - a.createdAt)[0]?.dataUrl || null;

  // The journey's plates in the order they were painted — the reel, and the
  // pool the chapter openers draw from.
  const seen = new Set();
  const frames = paints
  ...
  // ---- the chapters: one per spine beat that was actually walked --------
  ...
  const chapters = beatIndexes.map((beatIndex, chapterAt) => {
    const beat = beats[beatIndex] || {};
    const act = beat.act || 1;
    // The chapter WINDOW spans every sealed position of this beat (redacted
    // positions count for the span so entrances are not lost)…
    const positions = logs.map((log, i) => ({ log, turn: i })).filter(({ log }) => (log.beatIndex ?? 0) === beatIndex);
    const span = { from: positions[0]?.turn ?? 0, to: positions[positions.length - 1]?.turn ?? 0 };
    // …but struck CONTENT never reaches the book, whichever court struck it.
    const entries = positions.filter(({ log, turn }) => isBound(log, turn));
    const page = pages.find((p) => p.beatIndex === beatIndex) || null;
    const plate = openerPool.length ? openerPool[Math.min(openerPool.length - 1, Math.floor((chapterAt * openerPool.length) / beatIndexes.length))].dataUrl : null;
    ...
    return { beatIndex, numeral: romanNumeral(beatIndex + 1), title: beat.title || 'A Turn of the Road', act, page, plate, entries, entrances, dice };
  });
  ...
  const cast = (campaign.codex?.cast || []).map((soul) => `<article class="plate">${latestByLabel(soul.name) ? `<img src="${esc(latestByLabel(soul.name))}" alt="${esc(soul.name)}">` : `<div class="procedural-portrait large">${esc(initials(soul.name))}</div>`}<h3>${esc(soul.name)}</h3><p class="role">${esc(soul.role)}</p><p class="fate">${esc(fateLine(soul))}</p></article>`).join('');

// THE BRIEFING GATE (engine twin, pure fraction) — the DM is told one
// deterministic object each turn: the calendar line first, the scene-first
// pack (open threads riding inside the block), stated allegiances last and
// trimmed first. Same campaign, same bytes; the budget is a hard cap; the
// floors never trim.
...
console.log('PASS — the briefing gate (engine twin, pure fraction): calendar first, threads and allegiances ride, deterministic bytes, a hard budget, and floors that never trim.');

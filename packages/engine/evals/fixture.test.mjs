// THE FIXTURE GATE (Directive VIII.6–7, engine twin, pure fraction) —
// place-bound canon, sealed once like region canon. The door rules shape
// and atlas; the fold seals with a turn stamp (and, unlike the party, the
// offscreen tick MAY seal — the elsewhere builds, it never moves souls);
// the block carries fixture_state; the easel rides up to three fixtures
// of the painted place, most recently sealed first, byte-stable. Keyless,
// deterministic.
// STRIPPED: the schema-mirror fraction (reading server/dm.js source) is
// game/server law, judged at the table's own gate.
...
console.log('PASS — the fixture gate (engine twin, pure fraction): the door rules shape and atlas, the seal refuses rewrites, ticks may build the elsewhere, and the easel rides three sealed fixtures byte-stable; the schema mirror is judged at the table\'s own gate.');

// THE FRAME LAW GATE (Directive IX, engine twin, pure fraction) — the
// image cue is a claim about who stands in the painting, and the door
// judges the claim: the dead are not painted, the elsewhere is not
// painted, the unrecorded are not painted. Seated-court law throughout —
// each sub-court binds only where the briefing testifies, so the eval
// bench's bare {cast} context keeps its old leniency and the app's full
// landing context gets the full law. The two easel clauses are byte-stable:
// THE CLOSURE CLAUSE ends every subject-bearing scene brief, THE PRINCIPAL
// CLAUSE crowns the cue's first subject only when the roster paints a
// seatable soul by that name. The game addresses the easel law by its old
// in-game seat ('../src/lib/cinema/prompts.js', a shim re-exporting
// 'fatescript/cinema/prompts'); here we import the engine's true seat.
// The in-place growth check reads the engine's OWN visualBible gate (this
// package's sibling), which pins the same frame clauses.
import assert from 'node:assert/strict';
import fs from 'node:fs';
import { validateDmTurn, safeFallbackTurn } from '../src/protocol.js';
import { identityClause, scenePrompt } from '../src/cinema/prompts.js';
...
// The engine's OWN visualBible gate pins these clauses too — the growth is
// proven in place, against this package's sibling gate (not the table's).
...
console.log('PASS — the frame law gate (engine twin, pure fraction): the door refuses the dead, the elsewhere, and the unrecorded by name under seated-court law; crowd speaks only its two lawful words; the closure and principal clauses ride the easel byte-stable.');

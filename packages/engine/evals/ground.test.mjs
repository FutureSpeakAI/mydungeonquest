// THE GROUND GATE (engine twin, pure fraction; Directive VII.10) — the
// briefing names the ground by law. Byte-exact line, SECOND key, honest
// omission, name-only fallback, famine immunity, and the pack's two
// immunities for the standing region (the recent-text heuristic and the
// budget slim-trim are both bypassed).
import assert from 'node:assert/strict';
import { buildBriefing, buildContextPack } from '../src/graph.js';
...
console.log('PASS — the ground gate (engine twin, pure fraction): byte-exact second-key ground line, honest omission, name-only fallback, famine immunity, and the pack\'s full-ride and slim-trim immunities for the standing region.');

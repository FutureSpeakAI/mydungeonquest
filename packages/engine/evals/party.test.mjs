// THE PARTY GATE (Directive VIII, engine twin, pure fraction) — who
// travels with the hero, and who is elsewhere. Four courts at the door
// (membership, the living, the ground, the hero-root) plus the
// nobody-teleports law on speakers; the fold with its tick refusals; the
// pure replay with partyOf/elsewhereOf (travel moves members together, a
// leave pins the departed, a farewell in the leaving turn un-pins
// nobody); the briefing's traveling_with that never trims and an
// elsewhere that trims FIRST. Bare contexts get shape law only. Keyless,
// deterministic, fail-closed.
// STRIPPED: the server-door fraction (judgeTurn seating these courts from
// party_state/presence_state) and the schema-mirror fraction (reading
// server/dm.js source) are game/server law, judged at the table's own gate.
import assert from 'node:assert/strict';
import { validateDmTurn } from '../src/protocol.js';
import { applyStoryUpdates, initCodex, storyBlock } from '../src/story.js';
import { partyOf, elsewhereOf, presenceOf } from '../src/presence.js';
...
// The server door (judgeTurn seating these courts from party_state/
// presence_state) is game/server law, judged at the table's own gate.
...
// The schema-mirror fraction (reading server/dm.js source) is game/server
// law, judged at the table's own gate.

console.log('PASS — the party gate (engine twin, pure fraction): four courts at the door, nobody teleports, the fold refuses ticks, the replay pins the departed, the roster never trims, and the elsewhere falls first; the server door and schema mirror are judged at the table\'s own gate.');

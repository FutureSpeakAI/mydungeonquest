// THE PRESENCE GATE (Directive VII, engine twin, pure fraction) — the
// scene as sealed record, keyless. Four rooms: the door (the scene
// courts within validateDmTurn), the fold (the reducer's three answers),
// the block (scene_state rides to the server court), and the replay
// (presenceOf/visitorsOf, pure from the journal). The tick op set is
// proved closed on the way through. This gate is wholly pure; the game's
// server court (judgeTurn) is judged at the table's own gate.
import assert from 'node:assert/strict';
import { validateDmTurn } from '../src/protocol.js';
import { applyStoryUpdates, initCodex, storyBlock } from '../src/story.js';
import { presenceOf, visitorsOf } from '../src/presence.js';
import { tickUpdates } from '../src/livingWorld.js';
...
console.log('PASS — the presence gate (engine twin, pure fraction): the door\'s three scene courts, the fold\'s three answers, scene_state on the block, the closed tick op set, the pure replay with journal-row cites, and the fail-closed answer to a mangled record; the game\'s server court is judged at the table\'s own gate.');

// ------------------------------------------------------------
// THE SLATE GATE (engine twin, pure fraction) — the Market Law's
// first seat (Directive VI, Phase 3 groundwork).
//
// The slate is a projection of the record through the engine's one
// fold: seeds lock at first witness, drift moves one notch per sealed
// tick toward the pressure the region's scars show, a calm region
// rests, a struck tick moves nothing, an uncaused change is a price
// attack refused by name, and the same record reads the same slate
// byte for byte. Zero keys.
//
...
import { buildMarket, priceOf, slateLine, NOTCH, marketRowsFrom, regionSlate, STAPLES } from '../src/market.js';
...
// 5. THE FOLD'S SLATE — the slate speaks the engine's own words for the
//    active region (the ribbon that draws it is the table's own gate).
{
  ...
  assert.equal(slate.lines[0], slateLine(priceOf(market, 'Harrow Ford', STAPLES[0])), 'the slate adds no words of its own');
...
console.log('PASS \u2014 the slate gate (engine twin, pure fraction): staple seeds lock at first witness and replay byte for byte, drift walks one notch per sealed tick toward the scars\u2019 pressure while calm regions rest and struck ticks move nothing, an uncaused change is a price attack refused by name in the same fold, and the fold speaks its own slate lines for the active region; the folio\u2019s ribbon is judged at the table\u2019s own gate.');

// THE TABLE'S-DICE GATE (engine twin, pure fraction) — the table rolls
// only for the hero and the sheeted: with the sheet ledger seated,
// actor_id is 'hero' or a sheeted companion's name, refused by name
// otherwise; bare context keeps shape law. ONE roll engine, two lawful
// actors — the companion's die folds through the companion's own sheet
// and carries its owner's name; a death save is the plain d20 against
// 10. The enemy pool stays accounted. Keyless, deterministic. The
// game's compat seat '../src/lib/protocol.js' is a shim over
// 'fatescript/protocol'; the engine reads its true home '../src/protocol.js'.
// The prompt pin & schema mirror (game §V, reading '../src/lib/systemPrompt.js'
// and '../server/dm.js') are game-only law — the engine has no system
// prompt or tool schema — and are judged at the table's own gate.
import assert from 'node:assert/strict';
import { makeEntropy, validateDmTurn } from '../src/protocol.js';
import { companionRoll, heroRoll } from '../src/rules.js';
...
console.log('PASS — the table\u2019s dice (engine twin, pure fraction): the actor court, the sibling fold, the plain die, and the accounted pool hold; the prompt pin and schema mirror are judged at the table\u2019s own gate.');

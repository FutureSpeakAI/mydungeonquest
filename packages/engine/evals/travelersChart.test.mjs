// ---- THE TRAVELER'S CHART (engine twin, the pure fraction) ----
//
...
// paint. The rendered chart (medallions, roads, the vellum note) is
// React and is judged at the table's own gate; the engine has no
// window to render.
import assert from 'node:assert/strict';
import { readFileSync } from 'node:fs';
import { chartOf, chartRegions, dayLabel, travelsOf } from '../src/chart.js';

// The proving fixture lives at the table's own tree (read-only); the
// engine reads it by its settled path — the fold judges the same truths.
...
console.log('PASS — the traveler\'s chart gate (engine twin, pure fraction): roads only from lawful crossings, costs from the calendar alone, refusals hold, the vellum never guesses; the rendered chart is judged at the table\'s own gate');

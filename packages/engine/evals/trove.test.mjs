// THE TROVE GATE (engine twin, Directive VI) — named possessions as
// sealed operations. The validator refuses malformed writes and
// movements from hands the record does not show; the reducer guards
// canon with notes under the three-operation law; the story block tells
// the DM what is held; and the pure replay cites every hand a thing has
// passed through, seed included. The lib compat door (src/lib/*.js) is
// the table's own concern — the engine seats these folds itself.
...
import { troveOf, heldBy } from '../src/trove.js';
...
console.log('PASS — the trove gate (engine twin): possessions are sealed operations, unlawful hands are refused at two layers, the DM is told what is held, and every thing cites every hand it passed through; the lib compat door is judged at the table\u2019s own gate.');

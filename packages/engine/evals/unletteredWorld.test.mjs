// THE UNLETTERED-WORLD GATE (engine twin, pure fraction; 0.6.1, Task 53
// §2) — one clause instead of seven nouns. The paint law's accumulated
// silence vocabulary is retired; ONE general clause (the unlettered world)
// rides every builder exactly once, and the warden asks the text question
// of EVERY delivered plate: one repaint with the clause reinforced, then
// the plate is REFUSED and the refusal writes its note into the record.
// Identity still outranks text — a fallen face anchors, never refuses.
import assert from 'node:assert/strict';
import {
  UNLETTERED_WORLD, UNLETTERED_REINFORCEMENT, UNLETTERED_REFUSAL,
  unletteredBrief, parseUnlettered, unletteredRuling
} from '../src/unlettered.js';
import { wardenBrief, parseVerdict, mockWarden, wardenRuling } from '../src/warden.js';
import { applyStoryUpdates, initCodex } from '../src/story.js';
import { scrubPrompt, portraitPrompt, regionPrompt, scenePrompt, keyArtPrompt } from '../src/cinema/prompts.js';
...
console.log('PASS — the unlettered-world gate (engine twin, pure fraction): one clause rides every builder exactly once, the noun-pile is retired, the warden asks the text question of every plate, a second sighting refuses the plate, and the refusal writes its note into the record.');

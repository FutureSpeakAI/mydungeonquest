// THE WATCH GATE (engine twin, pure fraction) — the day divides into six
// named watches, four hours each, and the one calendar line wears the
// watch the sealed hours prove. The briefing carries it (and famine never
// starves it); the plate rides it beneath the beat and the mood; the
// keyless mock walks the clock through four watches of the first day.
// Deterministic, fail-closed on hours that are not lawful integers. This
// gate is wholly pure; nothing is stripped.
import assert from 'node:assert/strict';
import { WATCHES, watchOf, calendarOf, calendarLine } from '../src/calendar.js';
...
import { makeEntropy } from '../src/protocol.js';
...
console.log('PASS — the watch gate (engine twin, pure fraction): six watches stand the day, the line and the briefing wear them, the plate rides them, and the keyless mock walks four before dusk.');

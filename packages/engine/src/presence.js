...
function replay(campaign) {
  ...
  const heroName = String(campaign?.hero?.name || '').trim();
  ...
  (campaign?.logs || []).forEach((log, index) => {
    ...
    if (added && String(added).trim()) regions.add(canon(added));
    ...
    if (set && typeof set === 'object' && !Array.isArray(set) && regions.has(canon(set.region))) {
      ground = String(set.region).trim();
    }
    // Souls walk on before they speak: cast_add declares, then sights.
    for (const add of story.cast_add || []) {
      const name = String(add?.name || '').trim();
      if (!name) continue;
      if (!known.has(canon(name))) known.set(canon(name), name);
      sight(name, index);
    }
    for (const update of story.cast_update || []) if (update?.name) sight(update.name, index);
    for (const block of log?.dm?.narration_blocks || []) if (block?.speaker) sight(block.speaker, index);
    if (log?.dm?.dialogue_cue?.speaker) sight(log.dm.dialogue_cue.speaker, index);
    for (const add of story.item_add || []) if (add?.holder) sight(add.holder, index);
    for (const move of story.item_transfer || []) {
      if (move?.from) sight(move.from, index);
      if (move?.to) sight(move.to, index);
    }
    for (const drop of story.item_remove || []) if (drop?.holder) sight(drop.holder, index);
    for (const move of story.purse || []) if (move?.holder) sight(move.holder, index);
    // The hero is sighted by every non-struck player row.
    if (!log?.kind && heroName) sight(heroName, index);
  });
  return souls;
}

...
export function pickTurningPoint(entries = []) {
  ...
  entries.forEach((entry, index) => {
    if (entry.redacted || entry.kind === 'tick' || entry.kind === 'annal' || entry.kind === 'span' || entry.kind === 'epoch') return;
...
export function shelfModel({ worldTitle = '', taleTitle = '', covenant = '', cast = [], entries = [], forkedFrom = null } = {}) {
  return {
    ...
    passages: entries
      .filter((entry) => entry.kind !== 'tick' && entry.kind !== 'annal' && entry.kind !== 'span' && entry.kind !== 'epoch')
